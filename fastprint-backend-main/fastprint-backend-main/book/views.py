...
def admin_order_detail(request, pk):
    ...
    try:
        order = BookProject.objects.get(pk=pk, order_status='paid')
    ...
    serializer = BookProjectSerializer(order)
    return Response({
        'status': 'success',
...
# ===== NEW: Get all orders prepared for shipment =====
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def admin_shipment_orders(request):
    if not request.user.is_staff:
        return Response({"detail": "Not authorized."}, status=403)
    orders = BookProject.objects.filter(
        order_status='paid',
        is_shipment_prepared=True
    ).order_by("-created_at")
    serializer = BookProjectSerializer(orders, many=True)
    return Response({
        "status": "success",
...
# ===== NEW: Mark order as ready for shipment =====
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def prepare_shipment(request, pk):
    if not request.user.is_staff:
        return Response({"detail": "Not authorized."}, status=status.HTTP_403_FORBIDDEN)
    try:
        order = BookProject.objects.get(pk=pk, order_status='paid')
    ...
    order.is_shipment_prepared = True
    order.save(update_fields=['is_shipment_prepared'])

    serializer = BookProjectSerializer(order)
    return Response({
        'status': 'success',
        'message': 'Order added to shipment.',
        'data': serializer.data
...
# ===== UPDATED: Exclude shipment-prepared orders =====
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def admin_all_orders(request):
    ...
    orders = BookProject.objects.filter(
        order_status='paid',
        is_shipment_prepared=False  # ← Hides orders already in shipment
    ).order_by("-created_at")
...
def update_order_status(request, pk):
    ...
    return Response({
        ...
        'data': BookProjectSerializer(order).data
    }, status=status.HTTP_200_OK)