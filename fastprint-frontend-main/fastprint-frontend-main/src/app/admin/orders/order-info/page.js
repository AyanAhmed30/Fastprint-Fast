...
const OrderInfo = () => {
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        ...
        const res = await axios.get(`${BASE_URL}api/book/admin/orders/${orderId}/`, {
  ...
  const handleStatusChange = async (e) => {
    ...
    try {
      const token = localStorage.getItem("accessToken");
      await axios.patch(
        `${BASE_URL}api/book/orders/${orderData.id}/status/`,
        { status: newStatus },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleAddToShipment = async () => {
    ...
    try {
      const token = localStorage.getItem("accessToken");
      await axios.post(
        `${BASE_URL}api/book/admin/orders/${orderData.id}/prepare-shipment/`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      alert("Order added to shipment and removed from orders list.");
      router.push("/admin/orders");
    } catch (err) {
      console.error("Failed to add to shipment", err);
      alert("Failed to add order to shipment.");
    }
  };
  ...
  const InfoItem = ({ icon, label, value, isCategory = false }) => (
    <div className="flex items-start gap-3">
      ...
      <div className="flex-1">
        ...
        {isCategory ? (
          <span className="inline-flex items-center px-3 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 mt-1">
            {value || "—"}
          </span>
        ) : (
      ...
      return (
        <div className="border border-gray-200 rounded-lg p-4 hover:border-blue-300 transition-colors duration-200">
          ...
          <div className="flex gap-2">
            <a href={`${BASE_URL}${file}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors duration-200">
              <Eye className="w-4 h-4" /> View
            </a>
            <a href={`${BASE_URL}${file}`} download className="inline-flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-green-600 bg-green-50 rounded-lg hover:bg-green-100 transition-colors duration-200">
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-6">
            ...
            <select
              value={orderData.status || "in_progress"}
              onChange={handleStatusChange}
              disabled={savingStatus}
              className="w-full md:w-64 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 shadow-sm"
            >
              <option value="in_progress">In Progress</option>
              <option value="cancelled">Cancelled</option>
              <option value="shipped">Shipped</option>
            </select>
            {savingStatus && <p className="text-sm text-blue-600 mt-1">Saving...</p>}
          </div>
          ...
          <div className="mt-8">
            <InfoCard title="Shipping & Checkout Details" icon={<Package className="w-5 h-5" />} bgColor="from-blue-50 to-indigo-50">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <InfoItem icon={<User />} label="Order ID" value={orderData.id} />
          ...
          <div className="mt-8 text-center">
            <button
              ...
              className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-lg shadow hover:shadow-md hover:scale-105 transition-all duration-200 font-semibold"
            >
