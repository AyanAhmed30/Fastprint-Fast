...
const Orders = () => {
  ...
  const renderStatusBadge = (status) => {
    ...
    return (
      <div
        className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-gradient-to-r ${config.bgColor} ${config.textColor} border border-opacity-50`}
      >
        <div className={`w-2 h-2 ${config.dotColor} rounded-full mr-2 animate-pulse`}></div>
        {config.label}
      </div>
    );
  };
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-100">
                  {filteredProjects.length > 0 ? (
                    ...
                    <tr>
                      <td colSpan="4" className="px-6 md:px-8 py-12 md:py-16 text-center">
